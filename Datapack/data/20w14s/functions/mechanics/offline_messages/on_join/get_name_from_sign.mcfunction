...
# Adda temporary tag to the player.
tag @s add 20w14s.opm.target

# Store the players selector on a sign to let it resolve.
data modify block 731031 0 731030 Text1 set value '{"selector": "@p[tag=20w14s.opm.target]"}'

# Remove the temporary tag.
tag @s remove 20w14s.opm.target

# Store the name.
data modify storage 20w14s:mechanics/offline_messages root.NewElement.Name set from block 731031 0 731030 Text1
