...
execute if score @s 20w14s.id = 20w14s.opm.current_id temp run function 20w14s:mechanics/offline_messages/check_list/check_list_break
# Else, append to the output list and continue.
execute unless score @s 20w14s.id = 20w14s.opm.current_id temp run function 20w14s:mechanics/offline_messages/check_list/check_list_continue
