...
execute if score rocket_nearby temp matches 1 if score rocket_built temp matches 1 if score 20w14s.end_boss_ready global matches 1 as @e[type=villager,tag=20w14s.rocket_terminal_villager,sort=nearest,limit=1,distance=..8] at @s run function 20w14s:mechanics/rocket/launch/start