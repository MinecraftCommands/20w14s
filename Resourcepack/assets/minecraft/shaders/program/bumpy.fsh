...
void main(){
    ...
    vec4 nc = normalize(vec4(c.rgb, 1.0));
    vec4 nu = normalize(vec4(u.rgb, 1.0));
    vec4 nd = normalize(vec4(d.rgb, 1.0));
    vec4 nl = normalize(vec4(l.rgb, 1.0));
    vec4 nr = normalize(vec4(r.rgb, 1.0));
